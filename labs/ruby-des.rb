  ...
  class Block
    ...
    def initialize(input)
      if input.is_a?(String)
        ...
        @string    = input.join.lines.pack('B*')
        @bit_array = input
      else
...
class DES2
  ...
  def encrypt(data, key)
    # data = data.codepoints.to_a.pack("C*")
    blockinate data
    key  = RubyDES::Block.new key
    @blocks.each_index do |i|
      data = RubyDES::Block.new @blocks[i]
      des = RubyDES::Ctx.new(data, key)
      @blocks[i] = des.encrypt.string
    end
    # @encrypted_phrase = @blocks.join.unpack("C*").pack("U*")
